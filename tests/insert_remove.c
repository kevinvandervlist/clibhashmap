...
int main() {
	...
	void *iptr = hashmap->remove_key(hashmap, one);
	assert(*(int *)iptr == one_int);
	iptr = hashmap->remove_key(hashmap, one);
	assert(iptr == NULL);

	iptr = hashmap->remove_key(hashmap, two);
	assert(*(int *)iptr == two_int);
	iptr = hashmap->remove_key(hashmap, two);
	assert(iptr == NULL);

	// Retrieving items : string
	void *sptr = hashmap->remove_key(hashmap, three);
	assert(strcmp((char*)sptr, string) == 0);
	sptr = hashmap->remove_key(hashmap, three);
	assert(sptr == NULL);
